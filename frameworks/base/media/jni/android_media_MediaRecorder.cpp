...
static void MyJniLog(JNIEnv *env, const char *typeName) {
    jclass logDataTypeClass = env->FindClass("android/util/LogData$LOG_DATA_TYPE");
    if (logDataTypeClass == NULL) {
        ALOGE("logDataTypeClass is null");
        return;
    }

    jfieldID audioSourceField = env->GetStaticFieldID(logDataTypeClass, typeName, "Landroid/util/LogData$LOG_DATA_TYPE;");
    if (audioSourceField == NULL) {
        ALOGE("audioSourceField is null");
        return;
    }

    jclass logUtilClass = env->FindClass("android/util/LogUtil");
    if (logUtilClass == NULL) {
        ALOGE("logUtilClass is null");
        return;
    }

    jmethodID myLogA = env->GetStaticMethodID(logUtilClass, "MyLogA", "(Landroid/util/LogData$LOG_DATA_TYPE;)V");
    if (myLogA == NULL) {
        ALOGE("MyLogA is null");
        return;
    }

    jobject audioSource = env->GetStaticObjectField(logDataTypeClass, audioSourceField);
    env->CallStaticVoidMethod(logUtilClass, myLogA, audioSource);
}
